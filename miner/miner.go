...
func (m *Miner) mineOne(ctx context.Context, base *MiningBase) (*types.BlockMsg, error) {
	...
	win, proof, err := gen.IsRoundWinner(ctx, base.ts, int64(base.ts.Height()+base.nullRounds+1), m.addresses[0], &m.api)
	if err != nil {
...
func (m *Miner) scratchTicket(ctx context.Context, base *MiningBase) (*types.Ticket, error) {
	...
	vrfOut, err := m.computeVRF(ctx, vrfBase)
	if err != nil {
		return nil, err
	}
...
func (m *Miner) createBlock(base *MiningBase, ticket *types.Ticket, proof types.ElectionProof) (*types.BlockMsg, error) {
	...
	return m.api.MinerCreateBlock(context.TODO(), m.addresses[0], base.ts, ticket, proof, msgs, nheight, uint64(uts))
}
